...
function shareSlugSuffix(value) {
  const match = String(value || "").match(/([a-f0-9]{8})$/i);
...
export async function getDeckByShareToken(token) {
  const suffix = shareSlugSuffix(token);
  if (!hasDatabase()) {
    const decks = await readLocalDecks();
    return decks.find((deck) =>
      deck.shareToken === token ||
      deck.id === token ||
      (suffix && String(deck.id || "").toLowerCase().endsWith(`-${suffix}`))
    ) || null;
  }

  await ensureSchema();
  const rows = suffix
    ? await sql()`select ${deckColumns(sql())} from decks where share_token = ${token} or id = ${token} or id like ${`%-${suffix}`} limit 1`
    : await sql()`select ${deckColumns(sql())} from decks where share_token = ${token} or id = ${token} limit 1`;
  return rows[0] ? mapRow(rows[0]) : null;
}
